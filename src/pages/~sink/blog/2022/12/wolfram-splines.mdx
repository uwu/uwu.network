...
Simplify[
  ...
  D[cubicbezier[P3, P4, P5, P6, t - 1], {t, 3}],
  {
    t == 1,
    P4 == 2 P3 - P2,
    P5 == P1 + 4 P2 - 4 P3
...
- <K>P_0 + 12P_2 + P_6 = 6 P_1 + 8 P_3</K>

And put them in terms of <K>P_4</K>, <K>P_5</K>, and <K>P_6</K>:

- <K>P_4 = 2P_3-P_2</K>
- <K>P_5 = P_1 + 4P_3 - 4P_2</K>
- <K>P_6 = 6 P_1 + 8 P_3 - P_0 - 12 P_2</K>
...
So now, we have our constraints, so its time to prove that this implies <K>C^\infty</K>.

I will do this by showing that it is equivalent to continuing the original curve for <K>1 \le t \le 2</K>,
and since its the same curve, it must be continuous with itself!

So let's start with our constrained second half, and reduce it to the Bernstein form (it's just more compact):
...
And now try our original bezier curve, but with <K>t+1</K> instead of <K>t</K>
(so when we evaluate at <K>0.5</K>, thats not halfway along our curve, its <K>t=0.5+1=1.5</K>,
beyond the bounds of our curve):

<NotebookCell
  cells={[
    `cubicbezier[P0, P1, P2, P3, t + 1] // Expand;
Collect[%, {P0, P1, P2, P3}]`,
    `  P0 (                -   t^3)
+ P1 (          3 t^2 + 3 t^3)
+ P2 (  - 3 t - 6 t^2 - 3 t^3)
...
There! Increasing <K>t>1</K> is equivalent to adding more curves of <K>C^3</K>,
and as Beziérs are <K>C^\infty</K> with *themselves*, this means that <K>C^3 => C^\infty \square</K>.
